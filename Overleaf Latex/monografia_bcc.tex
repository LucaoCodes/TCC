...
\item \textbf{atividade 3:} Aplicação do modelo Causal Forest, com foco na inferência causal heterogênea. Será definido um tratamento exógeno e estimado o efeito causal condicional (CATE) sobre os retornos do Ibovespa em diferentes contextos econômicos.
...
Esses limites, longe de invalidar as conclusões alcançadas, delineiam caminhos naturais de extensão do trabalho. Entre eles, destacam-se a adoção de janelas adaptativas ou \textit{rolling} na avaliação \textit{out-of-sample}, a implementação plena do modelo de Causal Forests, já previsto no roteiro deste TCC, como tentativa explícita de aproximar a inferência causal a partir de aprendizado de máquina \cite{athey2019ml}, e o acompanhamento da regulamentação em curso na Autoridade Nacional de Proteção de Dados, cujos desdobramentos podem reorientar as exigências de explicabilidade aplicáveis a modelos quantitativos de risco no Brasil \cite{anpd2025nt12}.

A próxima etapa do trabalho será a estimação do modelo Causal Forest, com foco na inferência causal heterogênea. Os resultados desse experimento, bem como sua comparação com os demais modelos, serão incorporados a esta conclusão após a sua execução.
