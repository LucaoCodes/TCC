...
A estimação do GAM envolveu duas etapas de otimização sequenciais, ambas orientadas pelo MAE no conjunto de teste, para evitar que as escolhas de configuração favoreçam o ajuste \textit{in-sample}.
...
Para identificar o valor ótimo, foi realizada uma busca sistemática no conjunto $\{5, 10, 15, 20, 25\}$, treinando o GAM com todos os termos do tipo $s(\cdot)$ e avaliando o MAE no conjunto de teste para cada configuração. O valor com menor MAE \textit{out-of-sample} foi selecionado e utilizado nas etapas subsequentes.

\subsection{Seleção do tipo de termo por variável (\textit{smooth} vs.\ linear)}

Com o $n\_splines$ ótimo definido, foi conduzida uma análise variável a variável para determinar se a representação não linear — por meio de spline $s(\cdot)$ — gerava ganho de desempenho em relação a um termo linear $l(\cdot)$. Para cada variável $j$, o MAE \textit{out-of-sample} foi calculado mantendo-se aquela variável com o tipo em avaliação ($s$ ou $l$) e todas as demais com suas configurações de referência ($s$ com $n\_splines$ ótimo). O ganho não linear foi definido como:
...
Onde $\text{MAE}_{l(j)}$ e $\text{MAE}_{s(j)}$ representam o erro médio absoluto no conjunto de teste ao se utilizar, respectivamente, termo linear e termo suavizado para a variável $j$. Valores positivos de $\Delta\text{MAE}_j$ indicam que a não linearidade beneficia o ajuste; valores negativos ou nulos indicam que o termo linear é suficiente, e a variável é então representada como $l(\cdot)$.

O modelo GAM final foi construído com $s(\cdot, \, n\_splines=n^*)$ para variáveis com $\Delta\text{MAE}_j > 0$ e $l(\cdot)$ para as demais, simplificando o modelo sem custo preditivo. Os resultados do processo de otimização e o desempenho do modelo final são apresentados no Capítulo de Resultados.
...
O processo de otimização do GAM seguiu o procedimento descrito na Seção~\ref{sec:gam_otimizacao}. A Tabela~\ref{tab:gam_nsplines} apresenta os resultados da busca pelo número ótimo de splines, avaliando R² no conjunto de treino e MAE no conjunto de teste para cada valor do grid.

\begin{table}[ht!]
\centering
\begin{tabular}{|c|c|c|}
\hline
\textbf{$n\_splines$} & \textbf{R²\_treino} & \textbf{MAE\_teste} \\ \hline
5  & 0{,}2077 & 0{,}010372 \\ \hline
\textbf{10} & \textbf{0{,}2732} & \textbf{0{,}010156} \\ \hline
15 & 0{,}2844 & 0{,}010190 \\ \hline
20 & 0{,}2919 & 0{,}010246 \\ \hline
25 & 0{,}2964 & 0{,}010375 \\ \hline
\end{tabular}
\caption{Busca pelo número ótimo de splines no GAM: R² no treino e MAE no teste (negrito = configuração selecionada)}\label{tab:gam_nsplines}
\end{table}

O valor $n^* = 10$ produziu o menor MAE no conjunto de teste, sendo selecionado para o modelo final. Observa-se que valores crescentes de $n\_splines$ continuam elevando o R² de treino, mas deterioram o desempenho fora da amostra — padrão característico de sobreajuste que justifica o uso do conjunto de teste como critério de seleção.

Em seguida, foi conduzida a análise do tipo de termo por variável. A Tabela~\ref{tab:gam_termos} apresenta o MAE \textit{out-of-sample} para cada variável sob a representação linear $l(\cdot)$ e suavizada $s(\cdot)$, bem como o ganho não linear $\Delta\text{MAE}_j$ definido na Equação~\ref{eq:ganho_nao_linear}.
...
$\Delta$EMBI+ (Risco-País)   & 0{,}010386 & 0{,}010156 & $+$0{,}000230 & $s(\cdot)$ \\ \hline
$\Delta$Câmbio (Dólar)       & 0{,}010173 & 0{,}010156 & $+$0{,}000017 & $s(\cdot)$ \\ \hline
$\Delta$Expectativa IPCA     & 0{,}010159 & 0{,}010156 & $+$0{,}000003 & $s(\cdot)$ \\ \hline
Retorno PIB                  & 0{,}010149 & 0{,}010156 & $-$0{,}000007 & $l(\cdot)$ \\ \hline
$\Delta$Selic                & 0{,}010123 & 0{,}010156 & $-$0{,}000033 & $l(\cdot)$ \\ \hline
IPCA                         & 0{,}010098 & 0{,}010156 & $-$0{,}000058 & $l(\cdot)$ \\ \hline
\end{tabular}
\caption{Análise \textit{smooth} vs.\ linear por variável: $\Delta\text{MAE}_j > 0$ indica ganho não linear (tipo $s(\cdot)$ selecionado)}\label{tab:gam_termos}
\end{table}

Risco-país e câmbio foram as variáveis com maior ganho não linear, confirmando a existência de relações assimétricas e de saturação que o modelo linear não captura. As demais variáveis — IPCA, Selic e PIB — foram mantidas como termos lineares, simplificando o modelo sem custo preditivo.

Para uma comparação sistemática dos modelos, a Tabela~\ref{tab:metricas_split} apresenta as métricas \textit{in-sample} da linha de base lado a lado com a avaliação \textit{out-of-sample} no conjunto de teste (2020--2024).
...
GAM (final)    & 0{,}2935 & 0{,}010671 & 0{,}2938 & 0{,}010156 & 0{,}014375 \\ \hline
\end{tabular}
\caption{Comparativo completo de métricas: linha de base \textit{in-sample} (conjunto completo) e avaliação \textit{out-of-sample} (teste: jan/2020--jul/2024)}\label{tab:metricas_split}
\end{table}

Dois resultados se destacam. Primeiro, o GAM manteve R² praticamente idêntico entre treino (0{,}2732) e teste (0{,}2938), indicando ausência de sobreajuste e boa capacidade de generalização para um período estruturalmente distinto do utilizado na estimação. Segundo, o OLS também apresentou R² superior no conjunto de teste (0{,}2339) em relação ao valor \textit{in-sample} (0{,}2086), sugerindo que as relações lineares entre câmbio, risco-país e retornos do Ibovespa se intensificaram no período pós-2020 — marcado pela pandemia de COVID-19 e pelo ciclo de aperto monetário.

Essas falhas nos pressupostos do MQO reforçam o ganho explicativo obtido pelo modelo GAM. Para além das métricas agregadas, a análise dos efeitos marginais por meio de gráficos de dependência parcial (PDPs) permite compreender as relações não lineares capturadas pelo modelo.
...
Os gráficos de dependência parcial (PDPs) estimados para o modelo GAM final permitem visualizar o efeito marginal de cada variável macroeconômica sobre os retornos do Ibovespa, mantendo as demais constantes. A Figura~\ref{fig:gam_pdps} apresenta os PDPs das seis variáveis preditoras, com a faixa sombreada representando o intervalo de confiança de 95\%. PDPs mostram como a predição média do modelo varia ao longo da distribuição de uma variável, revelando não linearidades, assimetrias e efeitos de saturação \cite{Friedman2001Machine}.

\begin{figure}[htb]
    \begin{center}
        \includegraphics[scale=0.32]{gam_pdps.png}
    \end{center}
    \caption{\label{fig:gam_pdps}Partial Dependence Plots (PDPs) do GAM Final --- efeito marginal de cada variável sobre o retorno logarítmico do Ibovespa (Fonte: Autor)}
\end{figure}
...
A aplicação do modelo GAM confirmou empiricamente a capacidade de modelos aditivos de capturar relações não lineares que os modelos tradicionais não detectam. Com R² de 0{,}2938 no conjunto de teste (2020--2024), o GAM demonstrou não apenas ganho explicativo em relação ao MQO (R² = 0{,}2086 \textit{in-sample}), mas também boa capacidade de generalização para um período estruturalmente distinto do utilizado na estimação. A análise dos gráficos de dependência parcial (PDPs) revelou que câmbio e risco-país exercem efeitos assimétricos e não lineares sobre os retornos do Ibovespa: depreciações cambiais intensas têm impacto negativo crescente e desproporcional, enquanto aumentos no risco-país apresentam saturação parcial em choques extremos. O processo de otimização indicou que IPCA, Selic e PIB se comportam de forma aproximadamente linear no horizonte diário, resultado coerente com a hipótese de eficiência informacional para essas variáveis. A estabilidade do R² entre treino e teste evidencia ausência de sobreajuste, reforçando a validade do modelo como instrumento explicativo para além do período de estimação.

As próximas etapas concentram-se na estimação dos modelos BSTS (\textit{Bayesian Structural Time Series}) e Causal Forests, cujos dados já foram preparados e exportados. O BSTS introduzirá uma perspectiva dinâmica e probabilística, decompondo os retornos do Ibovespa em componentes estruturais e permitindo avaliar como a relevância das variáveis macroeconômicas evolui ao longo do tempo — em especial sob diferentes regimes de política monetária. O Causal Forest investigará a heterogeneidade causal dos choques: utilizando variações cambiais superiores ao percentil 75 como variável de tratamento, o modelo estimará o efeito causal condicional (CATE) sobre os retornos, identificando se esse efeito se altera entre períodos de alta e baixa Selic, diferentes níveis de risco-país ou outros contextos macroeconômicos. A técnica SHAP será aplicada aos resultados desses modelos para quantificar a contribuição individual de cada variável em diferentes regimes, complementando os PDPs obtidos no GAM.
...
Conclui-se, portanto, que o uso de IA interpretável representa não apenas uma evolução técnica, mas também uma exigência prática e ética em ambientes financeiros complexos. Os resultados obtidos até o presente — especialmente os achados do GAM sobre a assimetria cambial e a saturação do risco-país — já evidenciam o potencial desta abordagem para revelar estruturas econômicas que permanecem ocultas à modelagem linear. Com a estimação do BSTS e do Causal Forest, o trabalho avança para uma fase em que a dimensão dinâmica e causal dessas relações poderá ser investigada com maior profundidade, completando o quadro comparativo entre os modelos explicativos propostos.
