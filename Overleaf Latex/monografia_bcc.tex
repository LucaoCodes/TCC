...
Este capítulo apresenta os resultados obtidos ao longo do trabalho. A primeira seção consolida a linha de base estabelecida no TCC 1, com o tratamento dos dados, os testes de estacionariedade e a estimação dos modelos econométricos clássicos (ARIMA e MQO). Em seguida, são apresentados os resultados \textit{out-of-sample} dos modelos de IA interpretável incorporados no TCC 2 (GAM e BSTS), encerrando-se com uma síntese comparativa entre os modelos.
...
A marginalidade da componente sazonal observada na decomposição foi confirmada por uma análise de sensibilidade sobre o período de sazonalidade. Foram testadas configurações com sazonalidade igual a 5, 10, 21 e 22 pregões, mantendo-se fixos todos os demais hiperparâmetros do modelo final (B0+B7), e as diferenças em MAE \textit{out-of-sample} entre as quatro variantes foram inferiores a 0,2\%, com amplitudes das componentes sazonais decompostas da ordem de $10^{-3}$ em todos os casos. Para investigar se a série de retornos exibiria algum ciclo detectável que pudesse orientar a escolha do período, foram calculadas a função de autocorrelação (ACF), a autocorrelação parcial (PACF) e o espectro de potência da série de treino, apresentados na Figura~\ref{fig:bsts_saz_diag}: nenhum dos três diagnósticos revela padrão cíclico consistente nos lags de 5 ou 21 pregões, sendo os coeficientes de autocorrelação majoritariamente contidos na banda de IC 95\% e o periodograma aproximadamente plano no intervalo de frequências considerado. Esses resultados indicam que o efeito de calendário é pequeno independentemente do período adotado e que a série não apresenta sazonalidade estruturada detectável, tornando a escolha de cinco pregões defensável tanto pela convenção do ciclo semanal quanto pela equivalência empírica com as alternativas mensais testadas.

\begin{figure}[htb]
    \begin{center}
        \includegraphics[scale=0.40]{bsts_seasonality_diagnostico.png}
    \end{center}
    \caption{\label{fig:bsts_saz_diag}Diagnóstico de sazonalidade sobre a série de retornos logarítmicos do Ibovespa (treino: 2001--2019): ACF, PACF e periodograma com marcações nas frequências correspondentes a cinco pregões (semanal) e a vinte e um pregões (mensal) (Fonte: Autor)}
\end{figure}
